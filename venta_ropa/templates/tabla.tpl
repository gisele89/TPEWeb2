    ...
    <div class="container">
      <ul class="list-group">
            {foreach from=$Tareas item=tarea}
              ...
              {else}
                <li class="list-group-item">{$tarea['titulo']} ----- {$tarea['descripcion']}<a href="borrar/{$tarea['id']}">BORRAR</a> | <a href="completada/{$tarea['id']}">COMPLETADA| <a href="editar/{$tarea['id']}">EDITAR</a></li>
              {/if}
            {/foreach}
      </ul>
    </div>
